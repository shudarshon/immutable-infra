validate:
	...
	cd ${WORKSPACE}/terraform && /usr/local/bin/terraform validate
...
init:
	cd ${WORKSPACE}/terraform && /usr/local/bin/terraform init

apply:
	cd ${WORKSPACE}/terraform && /usr/local/bin/terraform apply -auto-approve

destroy:
	cd ${WORKSPACE}/terraform && /usr/local/bin/terraform destroy -auto-approve
